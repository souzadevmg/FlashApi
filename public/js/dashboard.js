...
function extractQueueTotal(queueData) {
    if (!queueData || typeof queueData !== 'object') {
        return 0;
    }

    const keys = ['pending', 'pendingMessages', 'queueSize', 'total', 'count', 'size'];
    for (const key of keys) {
        if (typeof queueData[key] === 'number') {
            return queueData[key];
        }
    }

    const numericValues = Object.values(queueData).filter((value) => typeof value === 'number');
...
async function safeGet(url, headers = {}) {
    try {
        ...
        return { ok: false, error: response.data?.message || 'Falha ao consultar endpoint' };
    } catch (error) {
        return { ok: false, error: error.response?.data?.message || error.message || 'Erro na requisicao' };
    }
}
...
async function loadAggregateMetrics() {
    setDashboardMessage('Consolidando dados de chats, contatos, grupos e mensagens...');

    const totals = {
        instances: instacias.length,
        connected: instacias.filter((instance) => instance.status === 'connected').length,
        chats: 0,
        contacts: 0,
        groups: 0,
        messages: 0,
    ...
    await Promise.all(instacias.map(async (instance) => {
        const headers = { apikey: instance.apikey };

        const [statsRes, queueRes] = await Promise.all([
            safeGet(`${apiurl}/api/config/stats`, headers),
            safeGet(`${apiurl}/api/chat/queue/status`, headers)
        ]);
        ...
        if (queueRes.ok) {
            totals.queue += extractQueueTotal(queueRes.data);
        }
    }));

    if (metricElements.totalInstances) metricElements.totalInstances.textContent = formatNumber(totals.instances);
    if (metricElements.connectedInstances) metricElements.connectedInstances.textContent = formatNumber(totals.connected);
    if (metricElements.chats) metricElements.chats.textContent = formatNumber(totals.chats);
    if (metricElements.contacts) metricElements.contacts.textContent = formatNumber(totals.contacts);
    if (metricElements.groups) metricElements.groups.textContent = formatNumber(totals.groups);
    if (metricElements.messages) metricElements.messages.textContent = formatNumber(totals.messages);
    if (metricElements.queue) metricElements.queue.textContent = formatNumber(totals.queue);
}
...
async function loadInstanceAnalytics(selectedApiKey) {
    ...
    const defaultApiKey = selectedApiKey || selectEl?.value || instacias[0]?.apikey;
    ...
    const chatsRows = chats.slice(0, 8).map((chat) => {
        const name = chat.nome || chat.name || chat.push_name || chat.subject || 'Sem nome';
    ...
    const contactsRows = contacts.slice(0, 8).map((contact) => {
        ...
        const number = contact.jid ? contact.jid.replace('@s.whatsapp.net', '') : '-';
    ...
    renderRows('tableRecentContacts', contactsRows, contactsRes.ok ? 'Sem contatos para exibir.' : 'Instancia desconectada ou sem acesso.');
    renderRows('tableRecentGroups', groupsRows, groupsRes.ok ? 'Sem grupos para exibir.' : 'Instancia desconectada ou sem acesso.');
...
async function loadSystemHealth() {
    ...
    const fallbackConnected = instacias.filter((instance) => instance.status === 'connected').length;

    if (listRes.ok) {
        const stats = listRes.data?.stats || {};
        if (totalEl) totalEl.textContent = formatNumber(stats.total || listRes.data?.total || fallbackTotal);
        if (connectedEl) connectedEl.textContent = formatNumber(stats.connected || fallbackConnected);
        if (connectingEl) connectingEl.textContent = formatNumber(stats.connecting || 0);
    } else {
    ...
    if (stateEl) {
        ...
        stateEl.className = healthy ? 'badge rounded-pill text-bg-success' : 'badge rounded-pill text-bg-danger';
    }
}
...
async function loadSystemInfra() {
    ...
    setTextById('systemCpuIdle', cpuLoad.idle == null ? '-' : formatPercent(cpuLoad.idle));

    setTextById('systemRamTotal', formatBytes(totalRamBytes));
    setTextById('systemRamUsed', `${formatBytes(calculatedUsedRamBytes)} (${formatPercent(ramUsagePercent)})`);
    setTextById('systemRamFree', formatBytes(availableRamBytes));
    setTextById('systemNodeHeap', `${formatBytes(processMem.heapUsed)} / ${formatBytes(processMem.heapTotal)}`);

    const disks = system.storage?.disks || [];
    const diskRows = disks.slice(0, 8).map((disk) => `
        <tr>
    ...
    const netRows = nets.slice(0, 8).map((net) => `
        <tr>
    ...
    renderRows('tableSystemNetwork', netRows, 'Nenhuma interface ativa encontrada.', 4);
}
...
async function refreshAnalytics() {
    ...
    try {
        ...
        setRealtimeStatus(`Ativo (${Math.floor(REALTIME_INTERVAL_MS / 1000)}s)`, true);
    } catch (error) {
        ...
        if (refreshBtn) {
            ...
            refreshBtn.innerHTML = '<i class="bi bi-arrow-clockwise me-1"></i> Atualizar';
        }
    }
}
...
function startRealtimeConsumptionUpdates() {
    ...
    realtimeIntervalId = setInterval(refreshRealtimeConsumption, REALTIME_INTERVAL_MS);
    setRealtimeStatus(`Ativo (${Math.floor(REALTIME_INTERVAL_MS / 1000)}s)`, true);
}
...
async function generateQRCode(instanceId, modal = true) {
    ...
    try {
        ...
        const response = await axios.put(`${apiurl}/api/session/conectar_sessao`, null, { headers });
...
async function disconnectInstance(instanceId) {
    ...
    try {
        ...
        const response = await axios.delete(`${apiurl}/api/session/desconect/${getapikey.apikey}`, { headers });
        if (response.data.success) {
...
async function deleteInstance(instanceId) {
    ...
    try {
        ...
        const response = await axios.delete(`${apiurl}/api/session/delete/${getapikey.apikey}`, { headers });
        if (response.data.success) {
            alert(response.data.message);
        } else {
...
$('#form-Criar-Instance').on('submit', async (event) => {
    ...
    try {
        ...
        const response = await axios.post(`${apiurl}/api/session/create_sessao`, data, { headers });
        if (response.data.success) {
